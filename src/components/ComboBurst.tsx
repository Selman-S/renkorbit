...
export function ComboBurst({ item, onDone }: ComboBurstProps) {
  ...
  const level = Math.max(item.combo, 1);
  const peakScale = 1.2 + Math.min(level, 12) * 0.12;
  ...
  return (
    <motion.div
      ...
      animate={{
        ...
        opacity: [0, 1, 1, 0],
        y: [24, -36 - level * 6, -108 - level * 10, -150 - level * 12],
      }}
      transition={{ duration: 0.72 + Math.min(level, 12) * 0.04, ease: 'easeOut', times: [0, 0.32, 0.62, 1] }}
      onAnimationComplete={() => onDone(item.id)}
    >
      <motion.span
        className="combo-burst__ring combo-burst__ring--outer"
        initial={{ scale: 0.4, opacity: 0.7 }}
        animate={{ scale: 1.8 + Math.min(level, 12) * 0.18, opacity: 0 }}
        transition={{ duration: 0.45 + Math.min(level, 12) * 0.03, delay: 0.18, ease: 'easeOut' }}
        aria-hidden
      />
      <motion.span
        className="combo-burst__ring combo-burst__ring--inner"
        initial={{ scale: 0.6, opacity: 0.9 }}
        animate={{ scale: 1.35 + Math.min(level, 12) * 0.12, opacity: 0 }}
        transition={{ duration: 0.38 + Math.min(level, 12) * 0.03, delay: 0.24, ease: 'easeOut' }}
        aria-hidden
      />
      <motion.span
        className="combo-burst__text"
        initial={{ scale: 0.8 }}
        animate={{ scale: [0.8, 1.08, 1.15 + Math.min(level, 12) * 0.03] }}
